...
function availableKeys(beforeIdx) {
  ...
  (S.classes || []).forEach((c, i) => { if (i < beforeIdx && c.name) out.push(c.name); });
  return out;
}
  ...
  S.classes.forEach((cl, idx) => {
    ...
    function asObjs() {
      if (!cl._objs || cl._dirtyKeys !== keys.join('|')) {
        cl._objs = (cl.rows || []).map((r) => {
          const o = {}; keys.forEach((_, j) => { o['k' + j] = r[j] == null ? '' : r[j]; });
    ...
    function syncBack() {
      cl.rows = asObjs().map((o) => {
        return keys.map((_, j) => { return o['k' + j]; }).concat([o.res]);
      });
    ...
    const editor = keys.length ? dataTable({
      ...
      rows: asObjs,
      blank: function () { const o = {}; keys.forEach((_, j) => { o['k' + j] = ''; }); o.res = ''; return o; },
      onChange: syncBack,
